...
export default function SubscriptionPlans() {
  ...
  const handleSelectPlan = async (plan: SubscriptionPlan) => {
    ...
    try {
      const response = await apiCall(allRoutes.subscriptions.request, "post", {
        planId: plan.id,
      });
  ...
  const getFeatureIcon = (feature: string) => {
    ...
    if (featureLower.includes("farmers") || featureLower.includes("users"))
      return <Users className="h-4 w-4" />;
    if (featureLower.includes("analytics") || featureLower.includes("reports"))
      return <BarChart3 className="h-4 w-4" />;
    if (featureLower.includes("api")) return <Code className="h-4 w-4" />;
    if (featureLower.includes("branding") || featureLower.includes("custom"))
      return <Palette className="h-4 w-4" />;
    if (featureLower.includes("support") || featureLower.includes("help"))
      return <Headphones className="h-4 w-4" />;
    if (featureLower.includes("manager") || featureLower.includes("dedicated"))
    ...
    if (featureLower.includes("priority") || featureLower.includes("24/7"))
              ...
              <CardContent className="space-y-4 flex-1 flex flex-col">
                ...
                <div className="space-y-3 pt-4 border-t">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-muted-foreground">
                      Core dairy management features
                    </span>
                  </div>
                  <div className="flex items-center gap-3">
                    <CheckCircle className="h-4 w-4 text-green-500" />
